Dynamic Multi-modal & Multi-event Survival Transformer
============================

Implementation scoped to `our_papers/Dynamic_Multi_modal_Multi_event_Survival_Transformer.pdf`.
**Read [SCOPE.md](SCOPE.md) first** - it maps every paper equation to its file, and records
what was removed from the upstream SAT codebase and why.

Quick start on Kaggle
------------

Upload either notebook from [notebooks/](notebooks/) to Kaggle and Run All. It clones
...
### Top-level directory layout
    ...
    ├── data                     # Data files (METABRIC bundled)
    ├── docs                     # Documentation
    ├── sat                      # Python source files
    ├── tests                    # Test suite
    ├── pyproject.toml           # Project definitions/dependencies
    ├── requirements-kaggle.txt  # Verified pins, with the reason for each
    ├── SCOPE.md                 # Paper -> code map; what was removed and why
    └── README.md
...
Or, without poetry (this is the set verified to work, with each non-obvious pin explained):
...
> Configuration of the tasks is done using Hydra.
    ...
    └── conf
          ...
          ├── sweep/                    # Optuna hyper-parameter search spaces
...
The loss is built compositionally, following Eq. 2 of the paper:

    Loss = Sum_i Sum_j L(y_ij, t_ij, e_ij) + Sum_l alpha_l * P_l

`MetaLoss` combines a list of loss components with a list of `coeffs` (the `alpha_l`).
Setting `balance_strategy` lets those weights be learned instead of fixed; `fixed`
reproduces the paper as written.

### Components in this tree

- **SATNLLPCHazardLoss** (`L_PCH`) - negative log-likelihood for piece-wise constant hazards
- **SampleRankingLoss** (`L_rank`, Eq. 4) - ranks observations within a single event type
- **MultiEventRankingLoss** (`L_mul`, Eq. 5) - ranks event types within a single observation
- **L1Loss** with `l1_type: margin` (`L_MAE`, Eq. 6) - best-guess MAE, Kaplan-Meier extended
- **MismatchLoss** (`L_MM`, Eq. 7) - penalises predicting the wrong first event
- **CrossEntropyLoss** / **MSELoss** - for the classification and regression heads

### The paper's recipe

    python -m sat.finetune experiments=<data>/survival         tasks/losses=nllpch_sample_event_ranking

See [docs/loss.md](docs/loss.md) for the ablation ladder and for why `L_mul` requires the
within-subject concordance metric rather than the standard C-index.
...
3. **Model Configurations**: Based on analysis results, reports the best-fitting event-time distribution per event
...
python -m sat.optuna_visualize --db data/optuna/metabric_survival.db --study metabric_survival_opt --output ./viz_results
```
