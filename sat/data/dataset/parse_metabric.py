...
@dataclass
class metabric:
    """Parse a DeepSurv-style train/test HDF5 survival dataset.

    The layout (train/{x,e,t} + test/{x,e,t}) is shared by METABRIC and SUPPORT,
    so both use this class; they differ only in which feature columns are
    categorical vs. continuous, which is supplied from the config.
    """
    ...
    # METABRIC defaults (9 features). SUPPORT overrides these in its config.
    cols_categorical: Optional[List[int]] = None
    cols_standardize: Optional[List[int]] = None
    ...
    def __call__(self) -> None:
        ...
        with h5py.File(self.source) as file:
            ...
            cols_categorical = list(
                ...
                if self.cols_categorical is not None
                else [4, 5, 6, 7]
            )
            cols_standardize = list(
                self.cols_standardize
                if self.cols_standardize is not None
                else [0, 1, 2, 3, 8]
            )
