...
def log_gpu_utilization():
    ...
    This is diagnostics only, so every failure path here is swallowed: NVML is
    frequently unavailable even when CUDA works (missing/mismatched nvml shared
    library, containers, WSL, the abandoned `nvidia-ml-py3` wheel). Letting that
    abort a training run would be absurd.
    """
    ...
    try:
        ...
        # torch can still report memory even when NVML cannot be reached
        try:
            used = torch.cuda.memory_allocated(0) // 1024 ** 2
            log.info(f"GPU memory allocated (torch): {used} MB. NVML unavailable: {e}")
        except Exception:
            log.info(f"GPU present but memory could not be queried: {e}")
