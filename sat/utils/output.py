...
def write_output(
    ...
    for i in range(cfg.data.num_events):
        ...
        if model.is_survival:
            ...
            # The discrete-hazard heads emit cuts + 1 columns, but the parametric
            # baselines (DSM/MENSA) evaluate on the full duration-cut grid and
            # emit one more. Hard-coding the width breaks those heads at the
            # prediction-dump step, after metrics have already been computed.
            n_cols = predictions[idx][:, i].shape[-1]
            columns = list(map(lambda c: "t" + str(c), range(n_cols)))
            ...
            risk_cols = list(
                map(lambda c: "t" + str(c), range(predictions[idx + 1][:, i].shape[-1]))
            )
            risk_df = pd.DataFrame(predictions[idx + 1][:, i], columns=risk_cols)
            risk_df["id"] = ids
            risk_df.to_csv(Path(f"{output_dir}/{prefix}risk{i}.csv"), index=False)
            surv_cols = list(
                map(lambda c: "t" + str(c), range(predictions[idx + 2][:, i].shape[-1]))
            )
            survival_df = pd.DataFrame(predictions[idx + 2][:, i], columns=surv_cols)
            survival_df["id"] = ids
            survival_df.to_csv(
