...
class DSMTaskHead(SurvivalTask):
    ...
    def forward(self, sequence_output, labels=None, **kwargs):
        ...
        # NOTE: hazard is deliberately NOT padded here. Hazard, risk and survival
        # are all evaluated on the same duration-cut grid, so they must stay the
        # same width. The previous pad_col made hazard one column wider, which
        # tripped the shape check in SurvivalEvaluationModule.survival_predictions;
        # that exception was swallowed and the Brier/IPCW metrics silently fell
        # back to their hard-coded 0.5 defaults. MENSA never padded and was fine.
