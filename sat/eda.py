...
def extract_dataset_components(cfg: DictConfig) -> Tuple:
    ...
    # Multi-event datasets store per-event lists in "events"/"durations" columns
    if "events" in df.columns and "durations" in df.columns:
        logger.info("Handling list-based multi-event dataset format")
        ...
        if "events" in df.columns and "durations" in df.columns:
            try:
                ...
                if isinstance(pdf["events"].iloc[0], list):
                    ...
                    logger.info(
                        f"Extracted {len(event_types)} events from list format"
                    )
                else:
        ...
        if len(durations) == 0 or len(events) == 0:
            logger.warning("No events extracted from the list-based format")
